...
namespace
{
    const QString DEFAULT_PEER_ID_CUSTOM = u""_s;
    const QString DEFAULT_USER_AGENT_CUSTOM = u""_s;
    ...
    const QString DEFAULT_STUN_SERVER = u"turn.cloud-rtc.com:80"_s;
    const QString DEFAULT_STUN_HTTP_SERVER = u"www.baidu.com"_s;
...
SessionImpl::SessionImpl(QObject* parent)
    ...
    , m_btProtocolTCP(BITTORRENT_SESSION_KEY(u"BTProtocolTCP"_s), BTProtocol_TCP::Both
        , clampValue(BTProtocol_TCP::Both, BTProtocol_TCP::None))
    , m_btProtocolUTP(BITTORRENT_SESSION_KEY(u"BTProtocolUTP"_s), BTProtocol_UTP::Both
        , clampValue(BTProtocol_UTP::Both, BTProtocol_UTP::None))
    ...
    , m_isStunEnabled{ BITTORRENT_SESSION_KEY(u"Stun/StunEnabled"_s), true }
    , m_StunServer{ BITTORRENT_SESSION_KEY(u"Stun/StunServer"_s), DEFAULT_STUN_SERVER }
    , m_StunHttpServer{ BITTORRENT_SESSION_KEY(u"Stun/StunHttpServer"_s),DEFAULT_STUN_HTTP_SERVER }
    , m_isStunSameIPv6PortEnabled{ BITTORRENT_SESSION_KEY(u"Stun/StunSameIPv6PortEnabled"_s), false }
    , m_CustomPeerFingerprint{ BITTORRENT_SESSION_KEY(u"Custom/PeerID"_s), DEFAULT_PEER_ID_CUSTOM }
    , m_CustomUserAgent{ BITTORRENT_SESSION_KEY(u"Custom/UserAgent"_s),DEFAULT_USER_AGENT_CUSTOM }
{
...
void SessionImpl::initializeNativeSession()
{
    ...
    LogMsg(tr("Custom Peer ID: \"%1\"").arg(getCustomPeerFingerprint()), Log::INFO);
    LogMsg(tr("HTTP User-Agent: \"%1\"").arg(USER_AGENT), Log::INFO);
    LogMsg(tr("Custom HTTP User-Agent: \"%1\"").arg(getCustomUserAgent()), Log::INFO);
...
lt::settings_pack SessionImpl::loadLTSettings() const
{
    ...
    // TCP 协议设置
    switch (btProtocolTCP())
    {
    case BTProtocol_TCP::Both:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_tcp, true);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_tcp, true);
        break;
    case BTProtocol_TCP::Incoming:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_tcp, true);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_tcp, false);
        break;
    case BTProtocol_TCP::Outgoing:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_tcp, false);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_tcp, true);
        break;
    case BTProtocol_TCP::None:
    default:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_tcp, false);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_tcp, false);
        break;
    ...
    // UTP 协议设置
    switch (btProtocolUTP())
    {
    case BTProtocol_UTP::Both:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_utp, true);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_utp, true);
        break;
    case BTProtocol_UTP::Incoming:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_utp, true);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_utp, false);
        break;
    case BTProtocol_UTP::Outgoing:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_utp, false);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_utp, true);
        break;
    case BTProtocol_UTP::None:
    default:
        settingsPack.set_bool(lt::settings_pack::enable_incoming_utp, false);
        settingsPack.set_bool(lt::settings_pack::enable_outgoing_utp, false);
        break;
    ...
    //stun
    settingsPack.set_bool(lt::settings_pack::enable_stun, isStunEnabled());
    settingsPack.set_str(lt::settings_pack::stun_server, getStunServer().toStdString());
    settingsPack.set_str(lt::settings_pack::stun_http_server, getStunHttpServer().toStdString());
    settingsPack.set_bool(lt::settings_pack::stun_same_ipv6_port, isStunSameIPv6PortEnabled());
    settingsPack.set_str(lt::settings_pack::custom_peer_fingerprint, getCustomPeerFingerprint().toStdString());
    settingsPack.set_str(lt::settings_pack::custom_user_agent, getCustomUserAgent().toStdString());
    return settingsPack;
}
...
BTProtocol_TCP SessionImpl::btProtocolTCP() const
{
    return m_btProtocolTCP;
}

void SessionImpl::setBTProtocolTCP(const BTProtocol_TCP protocol)
{
    if ((protocol < BTProtocol_TCP::Both) || (BTProtocol_TCP::None < protocol))
        return;

    if (protocol == m_btProtocolTCP) return;

    m_btProtocolTCP = protocol;
    configureDeferred();
}

BTProtocol_UTP SessionImpl::btProtocolUTP() const
{
    return m_btProtocolUTP;
}

void SessionImpl::setBTProtocolUTP(const BTProtocol_UTP protocol)
{
    if ((protocol < BTProtocol_UTP::Both) || (BTProtocol_UTP::None < protocol))
        return;

    if (protocol == m_btProtocolUTP) return;

    m_btProtocolUTP = protocol;
    configureDeferred();
}
...
QString SessionImpl::lastExternalIPv4Port() const
{
    return m_lastExternalIPv4Port;
}

QString SessionImpl::lastExternalIPv6Port() const
{
    return m_lastExternalIPv6Port;
}
...
void SessionImpl::handleAlert(lt::alert* alert)
{
    try
    {
        ...
        case lt::log_alert::alert_type:
            handleLogAlert(static_cast<const lt::log_alert*>(alert));
            break;
        case lt::my_log_alert::alert_type:
            handleMyLogAlert(static_cast<const lt::my_log_alert*>(alert));
            break;
        case lt::peer_log_alert::alert_type:
            handlePeerLogAlert(static_cast<const lt::peer_log_alert*>(alert));
            break;
        case lt::dht_log_alert::alert_type:
            handleDHTLogAlert(static_cast<const lt::dht_log_alert*>(alert));
            break;
...
void SessionImpl::handleExternalIPAlert(const lt::external_ip_alert* alert)
{
    ...
    const QString externalPort{ QString::number((alert->external_port)) };
    LogMsg(tr("Detected external Endpoint. IP: \"%1\" Port: \"%2\"")
        .arg(externalIP, externalPort), Log::INFO);
    ...
    if (isIPv6 && externalPort != m_lastExternalIPv6Port) {
        m_lastExternalIPv6Port = externalPort;
    }
    else if (isIPv4 && externalPort != m_lastExternalIPv4Port) {
        m_lastExternalIPv4Port = externalPort;
    }
...
void SessionImpl::handleLogAlert(const lt::log_alert* alert)
{
    LogMsg((tr("session_log: %1").arg(QString::fromStdString(alert->message()))), Log::INFO);
}

void SessionImpl::handleMyLogAlert(const lt::my_log_alert* alert)
{
    LogMsg((tr("%1").arg(QString::fromStdString(alert->message()))), Log::INFO);
}

void SessionImpl::handlePeerLogAlert(const lt::peer_log_alert* alert)
{
    LogMsg((tr("peer_log: %1").arg(QString::fromStdString(alert->message()))), Log::INFO);
}

void SessionImpl::handleDHTLogAlert(const lt::dht_log_alert* alert)
{
    LogMsg((tr("dht_log: %1").arg(QString::fromStdString(alert->message()))), Log::INFO);
}
...
void SessionImpl::setStunEnabled(bool value) {
    if (value != m_isStunEnabled)
    {
        m_isStunEnabled = value;
        configureDeferred();
    }
}
QString SessionImpl::getStunServer() const {
    return m_StunServer;
}
void SessionImpl::setStunServer(const QString& server) {
    if (server != m_StunServer)
    {
        m_StunServer = server;
        configureDeferred();
    }
}
QString SessionImpl::getStunHttpServer() const {
    return m_StunHttpServer;
}
void SessionImpl::setStunHttpServer(const QString& server) {
    if (server != m_StunHttpServer)
    {
        m_StunHttpServer = server;
        configureDeferred();
    }
}

bool SessionImpl::isStunSameIPv6PortEnabled() const {
    return m_isStunSameIPv6PortEnabled;
}
void SessionImpl::setStunSameIPv6PortEnabled(bool value) {
    if (value != m_isStunSameIPv6PortEnabled)
    {
        m_isStunSameIPv6PortEnabled = value;
        configureDeferred();
    }
}

//custom
QString SessionImpl::getCustomPeerFingerprint() const {
    return m_CustomPeerFingerprint;
}
void SessionImpl::setCustomPeerFingerprint(const QString& peerid) {
    QString result;
    for (const QChar& ch : peerid) {
        // 判断是否为 ASCII 字符（0x20 - 0x7E）
    ...
    if (result != m_CustomPeerFingerprint) {
        m_CustomPeerFingerprint = result;
        configureDeferred();
    }
}
QString SessionImpl::getCustomUserAgent() const {
    return m_CustomUserAgent;
}
void SessionImpl::setCustomUserAgent(const QString& userAgent) {
    if (userAgent != m_CustomUserAgent)
    {
